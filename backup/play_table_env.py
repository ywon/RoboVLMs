...
class PlayTableSimEnv(gym.Env):
    def __init__(
        ...
        #log.info(f"Using calvin_env with commit {get_git_commit_hash(Path(__file__))}.")
...
def get_env(dataset_path, obs_space=None, show_gui=True, **kwargs):
    ...
    asset_root = str(Path(__file__).parents[2] / "data")

    # interpolation 말고 실제 문자열로 직접 박기
    render_conf["data_path"] = asset_root

    if "scene" in render_conf:
        render_conf["scene"]["data_path"] = asset_root

    # evaluation에서는 recording/vr 흔적 비활성화
    render_conf["show_gui"] = show_gui
    render_conf["use_vr"] = False
    if "record" in render_conf:
        render_conf["record"] = False
    if "save_dir" in render_conf:
        render_conf["save_dir"] = str(Path(dataset_path))
    if "load_dir" in render_conf and render_conf["load_dir"] is None:
        render_conf["load_dir"] = str(Path(dataset_path))
    ...
    if "scene" in kwargs:
        scene_cfg = OmegaConf.load(
            Path(__file__).parents[2] / "conf" / "scene" / f"{kwargs['scene']}.yaml"
        )
        render_conf = OmegaConf.merge(render_conf, scene_cfg)
        render_conf["data_path"] = asset_root
        render_conf["scene"]["data_path"] = asset_root
    ...
    print("render_conf.data_path =", render_conf.get("data_path", None))
    print("render_conf.scene.data_path =", render_conf["scene"].get("data_path", None))
    print("resolved scene =", OmegaConf.to_yaml(render_conf["scene"], resolve=True))
